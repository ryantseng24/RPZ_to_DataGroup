...
def process_fqdn_zones():
    """
    处理FQDN类型的RPZ zones，为每个zone创建单独的文件
    """
    ...
    for zone in zones:
        ...
        # 为每个zone创建单独的文件
        output_file = os.path.join(OUTPUT_DIR, f"{zone}_blacklist.txt")
        write_datagroup_file(entries, output_file)
        logger.info(f"成功创建 {zone} zone的文件: {output_file}")

def process_ip_zones():
    """
    处理IP类型的RPZ zones，为每个zone创建单独的文件
    """
